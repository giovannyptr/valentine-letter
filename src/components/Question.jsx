...
function Question() {
  ...
  const trends = ['Why is my bf so cute?', 'Why does Gio love him more?', `Gio's birthdate (coz her bf forgets)`,]

  const handleSearch = (e) => {
    e.preventDefault()
    if (searchQuery.toLowerCase() === 'how long we have not seen each other?' || searchQuery.toLowerCase() === 'how long we have not seen each other') {
          navigate("/timer");
       ...
       <div
        ...
        <div className="flex flex-col min-h-screen w-full max-w-[480px] mx-auto ">
          ...
          <div className='mt-10  px-5 text-white/50'>
            <p>Pro tip: try searching "How long we have not seen each other?" ;)</p>
          </div>
        </div>
      )}
    </div>
