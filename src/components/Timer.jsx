...
function Timer() {
  // Last meeting date
  const LAST_MEETING_DATE = new Date('2024-03-10');
  const [timeApart, setTimeApart] = useState({
    days: 0,
    hours: 0,
    minutes: 0,
    seconds: 0
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const days = Math.abs(differenceInDays(now, LAST_MEETING_DATE));
      const hours = Math.abs(differenceInHours(now, LAST_MEETING_DATE) % 24);
      const minutes = Math.abs(differenceInMinutes(now, LAST_MEETING_DATE) % 60);
      const seconds = Math.abs(differenceInSeconds(now, LAST_MEETING_DATE) % 60);

      setTimeApart({ days, hours, minutes, seconds });
    }, 1000);
      ...
      <div className="text-center z-10">
        ...
        <div className="flex items-center justify-center gap-3 sm:gap-4 text-5xl sm:text-7xl font-bold mb-6">
          <div className="flex flex-col items-center">
            <span className="drop-shadow-lg">{timeApart.days.toString().padStart(2, '0')}</span>
            <span className="text-sm sm:text-2xl mt-1 sm:mt-2">Days</span>
          </div>
          <span className="text-4xl sm:text-6xl">:</span>
          <div className="flex flex-col items-center">
            <span className="drop-shadow-lg">{timeApart.hours.toString().padStart(2, '0')}</span>
            <span className="text-sm sm:text-2xl mt-1 sm:mt-2">Hours</span>
          </div>
          <span className="text-4xl sm:text-6xl">:</span>
          <div className="flex flex-col items-center">
            <span className="drop-shadow-lg">{timeApart.minutes.toString().padStart(2, '0')}</span>
            <span className="text-sm sm:text-2xl mt-1 sm:mt-2">Minutes</span>
          </div>
          <span className="text-4xl sm:text-6xl">:</span>
          <div className="flex flex-col items-center">
            <span className="drop-shadow-lg">{timeApart.seconds.toString().padStart(2, '0')}</span>
            <span className="text-sm sm:text-2xl mt-1 sm:mt-2">Seconds</span>
          </div>
        </div>

        <p className='text-lg mb-8 drop-shadow-lg'>{`... and I miss you every second <3` }</p>
